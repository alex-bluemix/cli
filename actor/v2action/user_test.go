			...
			Context("when we get a 409 status code because the user already exists", func() {
				BeforeEach(func() {
					fakeUAAClient.NewUserReturns(
						uaa.User{},
						// TODO: update error
						errors.New("this error needs to be updated to the proper struct type returned by the uaa client"),
					)
				})

				PIt("does not return an error and returns a user already exists warning", func() {
					Expect(actualErr).To(MatchError("foo"))
					Expect(actualWarnings).To(ConsistOf("bar"))
				})
			})
